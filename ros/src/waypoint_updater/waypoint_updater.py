...
class WaypointUpdater(object):
    def __init__(self):
        ...
        # doneTODO: Add other member variables you need below
        self.pose = None
        self.base_lane = None
    ...
    def loop(self):
        rate = rospy.Rate(20) # DEBUG: can lower from 50Hz, for testing purposes
...
#                 closest_waypoint_idx = self.get_closest_waypoint_idx()
    ...
    def get_closest_waypoint_idx(self):
        ...
        # If closest waypoint is behind us, take the next one
    ...
    def decelerate_waypoints(self, waypoints, closest_idx):
        ...
        for i, wp in enumerate(waypoints):
            ...
            curr_vel = wp.twist.twist.linear.x   # self.get_waypoint_velocity(wp)
            vel = math.sqrt(2 * MAX_DECEL * dist)
            if vel < 1.:
                vel = 0.

            p.twist.twist.linear.x = min(vel, curr_vel)
            temp.append(p)
